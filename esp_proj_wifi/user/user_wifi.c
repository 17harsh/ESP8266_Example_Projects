...
static bool isSleepTimerArmed = false;
...
//		if(!isSleepTimerArmed){
//			// Put esp8266 in deep sleep
//			//ArmSleepTimer();
//		}
		break;
	...
	case EVENT_STAMODE_GOT_IP:
		...
		ret = system_os_post(USER_TASK_PRIO_2, PING_INTERNET, 0);
...
void ICACHE_FLASH_ATTR _Ping_recv_cb (void* arg, void *pdata){
	...
	if(pingNo >= 4){
		if(total_payload >= 96){
			// Switch OFF LOS LED
			GPIO_OUTPUT_SET(GPIO_ID_PIN(LOS_LED), 0);
		}
		else{
			// Switch ON LOS LED
			GPIO_OUTPUT_SET(GPIO_ID_PIN(LOS_LED), 1);
...
void ICACHE_FLASH_ATTR _WifiUserTasks(os_event_t *event){
	WIFI_DEBUG_ARGS("Inside ChangeOpMode user task, event : %d", event->sig);
...
bool ICACHE_FLASH_ATTR _InitWifiStatusLEDs(){
	WIFI_DEBUG("Initializing LOS LED GPIO.");
 ...
 * 						 false if failed
 ******************************************************************************************/
bool ICACHE_FLASH_ATTR InitWifi(void){
	bool ret = false;
